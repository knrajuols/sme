...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    const upstream = await fetch(`${TENANT_SVC}/dashboard/summary`, {
      method: 'GET',
      headers: upstreamHeaders(req, 'GET /dashboard/summary'),
      cache: 'no-store',
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF GET /dashboard/summary] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-DASH-5001]', extractMsg(body, 'Failed to load dashboard summary'), upstream.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF GET /dashboard/summary] fetch failed:', msg);
    return err('[ERR-DASH-5001]', msg, 503);
  }
}
