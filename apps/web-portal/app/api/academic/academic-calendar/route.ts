...
// ── GET /api/academic/academic-calendar?academicYearId=... ──────────────────
export async function GET(req: NextRequest): Promise<NextResponse> {
  const qs = req.nextUrl.searchParams.toString();
  const url = `${TENANT_SVC}/academic/academic-calendar${qs ? `?${qs}` : ''}`;
  try {
    const up = await fetch(url, {
      method: 'GET',
      headers: upstreamHeaders(req, 'GET /academic/academic-calendar'),
      cache: 'no-store',
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) {
      console.error('[BFF GET /academic/academic-calendar] upstream', up.status, JSON.stringify(body));
      return err('[ERR-PT-CAL-5001]', extractMsg(body, 'Failed to list calendar entries'), up.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF GET /academic/academic-calendar] fetch failed:', msg);
    return err('[ERR-PT-CAL-5001]', msg, 503);
  }
}
