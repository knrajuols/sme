...
// ── POST /api/academic/periods/seed-from-master ───────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    const upstream = await fetch(`${TENANT_SVC}/academic/periods/seed-from-master`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/periods/seed-from-master'),
      body: '{}',
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(
        `[BFF POST /academic/periods/seed-from-master] upstream ${upstream.status}:`,
        JSON.stringify(body),
      );
      return err('[ERR-PER-SEED-MASTER]', extractMsg(body, 'Failed to seed periods from master template'), upstream.status);
    }
    return NextResponse.json(body, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /academic/periods/seed-from-master] fetch failed:', msg);
    return err('[ERR-PER-SEED-MASTER-503]', msg, 503);
  }
}
