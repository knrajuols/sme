...
// ── POST /api/academic/periods/seed ──────────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    const upstream = await fetch(`${TENANT_SVC}/academic/periods/seed`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/periods/seed'),
      body: '{}',
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF POST /academic/periods/seed] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-PER-SEED]', extractMsg(body, 'Failed to seed periods'), upstream.status);
    }
    return NextResponse.json(body, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /academic/periods/seed] fetch failed:', msg);
    return err('[ERR-PER-SEED-503]', msg, 503);
  }
}
