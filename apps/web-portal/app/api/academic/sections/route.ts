...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const url = `${TENANT_SVC}/academic/sections${qs ? `?${qs}` : ''}`;
  try {
    ...
    if (!upstream.ok) {
      ...
      return err('[ERR-SEC-5001]', extractMsg(body, 'Failed to list sections'), upstream.status);
    }
    ...
    return err('[ERR-SEC-5001]', msg, 503);
  }
}
