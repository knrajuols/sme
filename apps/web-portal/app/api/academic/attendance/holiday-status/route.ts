...
/** GET — Fetch holiday status for a specific date */
export async function GET(req: NextRequest): Promise<NextResponse> {
  const qs = new URL(req.url).searchParams.toString();
  const url = `${TENANT_SVC}/academic/attendance/holiday-status${qs ? `?${qs}` : ''}`;
  try {
    const up = await fetch(url, {
      method: 'GET',
      headers: upstreamHeaders(req, 'GET /academic/attendance/holiday-status'),
      cache: 'no-store',
    });
    const body: unknown = await up.json().catch(() => null);
    if (!up.ok) return err('[ERR-AHS-5001]', extractMsg(body, 'Failed to fetch holiday status'), up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    return err('[ERR-AHS-5001]', e instanceof Error ? e.message : 'Unavailable', 503);
  }
}
