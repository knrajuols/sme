...
/** GET — Fetch attendance aggregation report */
export async function GET(req: NextRequest): Promise<NextResponse> {
  const qs = new URL(req.url).searchParams.toString();
  const url = `${TENANT_SVC}/academic/attendance/report${qs ? `?${qs}` : ''}`;
  try {
    const up = await fetch(url, {
      method: 'GET',
      headers: upstreamHeaders(req, 'GET /academic/attendance/report'),
      cache: 'no-store',
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) return err('[ERR-ATR-5001]', extractMsg(body, 'Failed to fetch attendance report'), up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    return err('[ERR-ATR-5001]', e instanceof Error ? e.message : 'Unavailable', 503);
  }
}
