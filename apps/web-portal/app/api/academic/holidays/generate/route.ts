...
// ── POST /api/academic/holidays/generate ────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  const url = `${TENANT_SVC}/academic/holidays/generate`;
  try {
    const payload = await req.json();
    const up = await fetch(url, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/holidays/generate'),
    ...
    if (!up.ok) {
      ...
      return err('[ERR-PT-HOL-GEN-5001]', extractMsg(body, 'Failed to generate holidays'), up.status);
    }
    ...
    return err('[ERR-PT-HOL-GEN-5001]', msg, 503);
  }
}
