...
// ── POST /api/academic/holidays/preview ─────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  const url = `${TENANT_SVC}/academic/holidays/preview`;
  try {
    const payload = await req.json();
    const up = await fetch(url, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/holidays/preview'),
    ...
    if (!up.ok) {
      ...
      return err('[ERR-PT-HOL-PRV-5001]', extractMsg(body, 'Failed to preview holidays'), up.status);
    }
    ...
    return err('[ERR-PT-HOL-PRV-5001]', msg, 503);
  }
}
