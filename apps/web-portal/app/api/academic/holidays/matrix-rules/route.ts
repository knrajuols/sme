...
// ── GET /api/academic/holidays/matrix-rules?academicYearId=... ──────────────
export async function GET(req: NextRequest): Promise<NextResponse> {
  const qs = req.nextUrl.searchParams.toString();
  const url = `${TENANT_SVC}/academic/holidays/matrix-rules${qs ? `?${qs}` : ''}`;
  try {
    const up = await fetch(url, {
      method: 'GET',
      headers: upstreamHeaders(req, 'GET /academic/holidays/matrix-rules'),
      cache: 'no-store',
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) {
      console.error('[BFF GET /holidays/matrix-rules] upstream', up.status, JSON.stringify(body));
      return err('[ERR-PT-HOL-MR-5001]', extractMsg(body, 'Failed to load matrix rules'), up.status);
    }
    ...
    return err('[ERR-PT-HOL-MR-5001]', msg, 503);
  }
}

// ── POST /api/academic/holidays/matrix-rules ────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  const url = `${TENANT_SVC}/academic/holidays/matrix-rules`;
  try {
    const payload = await req.json();
    const up = await fetch(url, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/holidays/matrix-rules'),
      body: JSON.stringify(payload),
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) {
      console.error('[BFF POST /holidays/matrix-rules] upstream', up.status, JSON.stringify(body));
      return err('[ERR-PT-HOL-MR-5002]', extractMsg(body, 'Failed to save matrix rules'), up.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /holidays/matrix-rules] fetch failed:', msg);
    return err('[ERR-PT-HOL-MR-5002]', msg, 503);
  }
}
