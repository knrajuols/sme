...
// ── GET /api/academic/classes?academicYearId=... ──────────────────────────────
export async function GET(req: NextRequest): Promise<NextResponse> {
  const { searchParams } = new URL(req.url);
  const qs = searchParams.toString();
  const url = `${TENANT_SVC}/academic/classes${qs ? `?${qs}` : ''}`;
  try {
    ...
    if (!upstream.ok) {
      ...
      return err('[ERR-CLS-5001]', extractMsg(body, 'Failed to list classes'), upstream.status);
    }
    ...
    return err('[ERR-CLS-5001]', msg, 503);
  }
}
