...
// ── POST /api/academic/clone/academic-calendar ──────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  const url = `${TENANT_SVC}/academic/clone/academic-calendar`;
  try {
    const payload = await req.json();
    const up = await fetch(url, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/clone/academic-calendar'),
      body: JSON.stringify(payload),
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) {
      console.error('[BFF POST /clone/academic-calendar] upstream', up.status, JSON.stringify(body));
      return err('[ERR-PT-CLN-CAL-5001]', extractMsg(body, 'Failed to clone calendar from master'), up.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /clone/academic-calendar] fetch failed:', msg);
    return err('[ERR-PT-CLN-CAL-5001]', msg, 503);
  }
}
