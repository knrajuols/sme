...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const url = `${TENANT_SVC}/academic/marks${qs ? `?${qs}` : ''}`;
  try {
    const up = await fetch(url, { method: 'GET', headers: upstreamHeaders(req, 'GET /academic/marks'), cache: 'no-store' });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) return err('[ERR-MARK-5001]', extractMsg(body, 'Failed to fetch marks'), up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) { return err('[ERR-MARK-5001]', e instanceof Error ? e.message : 'Unavailable', 503); }
}
