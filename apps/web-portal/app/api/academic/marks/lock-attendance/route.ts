...
export async function POST(req: NextRequest): Promise<NextResponse> {
  let payload: unknown;
  try { payload = await req.json(); } catch { return err('[ERR-LOCK-5001]', 'Invalid JSON body', 400); }
  try {
    const up = await fetch(`${TENANT_SVC}/academic/marks/lock-attendance`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/marks/lock-attendance'),
      body: JSON.stringify(payload),
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) return err('[ERR-LOCK-5001]', extractMsg(body, 'Failed to lock attendance'), up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) { return err('[ERR-LOCK-5001]', e instanceof Error ? e.message : 'Unavailable', 503); }
}
