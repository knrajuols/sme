...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const url = `${TENANT_SVC}/academic/marks/attendance-lock${qs ? `?${qs}` : ''}`;
  try {
    const up = await fetch(url, {
      method: 'GET',
      headers: upstreamHeaders(req, 'GET /academic/marks/attendance-lock'),
      cache: 'no-store',
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) return err('[ERR-LOCK-5002]', extractMsg(body, 'Failed to fetch lock status'), up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) { return err('[ERR-LOCK-5002]', e instanceof Error ? e.message : 'Unavailable', 503); }
}
