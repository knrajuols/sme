...
// ── GET /api/academic-setup/class-sections?classId=... ────────────────────────
export async function GET(req: NextRequest): Promise<NextResponse> {
  const { searchParams } = new URL(req.url);
  const classId = searchParams.get('classId');
  const qs = classId ? `?classId=${encodeURIComponent(classId)}` : '';
  try {
    const upstream = await fetch(`${TENANT_SVC}/academic/class-sections${qs}`, {
      method: 'GET',
    ...
    if (!upstream.ok) {
      ...
      return err('[ERR-AC-CSC-5001]', extractMsg(body, 'Failed to list class-sections'), upstream.status);
    }
    ...
    return err('[ERR-AC-CSC-5001]', msg, 503);
  }
}

// ── POST /api/academic-setup/class-sections ───────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  let payload: unknown;
  try {
    payload = await req.json();
  } catch {
    return err('[ERR-AC-CSC-5002]', 'Invalid JSON body', 400);
  }
  try {
    const upstream = await fetch(`${TENANT_SVC}/academic/class-sections`, {
      method: 'POST',
    ...
    if (!upstream.ok) {
      ...
      return err('[ERR-AC-CSC-5002]', extractMsg(body, 'Failed to create class-section'), upstream.status);
    }
    return NextResponse.json(body, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /academic/class-sections] fetch failed:', msg);
    return err('[ERR-AC-CSC-5002]', msg, 503);
  }
}
