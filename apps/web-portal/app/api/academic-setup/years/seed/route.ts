...
// ── POST /api/academic-setup/years/seed ───────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    const upstream = await fetch(`${TENANT_SVC}/academic/years/seed`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/years/seed'),
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF POST /academic/years/seed] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-AC-YR-SEED]', extractMsg(body, 'Failed to seed academic years'), upstream.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /academic/years/seed] fetch failed:', msg);
    return err('[ERR-AC-YR-SEED]', msg, 503);
  }
}
