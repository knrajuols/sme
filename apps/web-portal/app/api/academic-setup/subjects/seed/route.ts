...
// ── POST /api/academic-setup/subjects/seed ────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    const upstream = await fetch(`${TENANT_SVC}/academic/subjects/seed`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/subjects/seed'),
      body: JSON.stringify({}),
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF POST /academic/subjects/seed] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-AC-SUBJ-SEED]', extractMsg(body, 'Failed to seed subjects'), upstream.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /academic/subjects/seed] fetch failed:', msg);
    return err('[ERR-AC-SUBJ-SEED]', msg, 503);
  }
}
