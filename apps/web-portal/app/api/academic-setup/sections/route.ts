...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!upstream.ok) {
      console.error(`[BFF GET /academic/sections] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-AC-SEC-5001]', extractMsg(body, 'Failed to list sections'), upstream.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF GET /academic/sections] fetch failed:', msg);
    return err('[ERR-AC-SEC-5001]', msg, 503);
  }
}

// ── POST /api/academic-setup/sections ────────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  let payload: unknown;
  try {
    payload = await req.json();
  } catch {
    return err('[ERR-AC-SEC-5002]', 'Invalid JSON body', 400);
  }
  try {
    const upstream = await fetch(`${TENANT_SVC}/academic/sections`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /academic/sections'),
      body: JSON.stringify(payload),
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF POST /academic/sections] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-AC-SEC-5002]', extractMsg(body, 'Failed to create section'), upstream.status);
    }
    return NextResponse.json(body, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /academic/sections] fetch failed:', msg);
    return err('[ERR-AC-SEC-5002]', msg, 503);
  }
}
