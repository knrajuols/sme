...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const upstream = await fetch(`${TENANT_SVC}/finance/invoices/generate`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /finance/invoices/generate'),
      body: JSON.stringify(payload),
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF POST /finance/invoices/generate] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-FIN-INV-5001]', extractMsg(body, 'Failed to generate invoices'), upstream.status);
    }
    return NextResponse.json(body, { status: upstream.status });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /finance/invoices/generate] fetch failed:', msg);
    return err('[ERR-FIN-INV-5001]', msg, 503);
  }
}
