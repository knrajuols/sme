...
// ── GET /api/finance/invoices?classId=&studentId= ─────────────────────────────
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const url = `${TENANT_SVC}/finance/invoices${qsStr ? `?${qsStr}` : ''}`;

    const upstream = await fetch(url, {
      method: 'GET',
      headers: upstreamHeaders(req, 'GET /finance/invoices'),
      cache: 'no-store',
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF GET /finance/invoices] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-FIN-INV-5002]', extractMsg(body, 'Failed to list invoices'), upstream.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF GET /finance/invoices] fetch failed:', msg);
    return err('[ERR-FIN-INV-5002]', msg, 503);
  }
}
