...
export async function GET(
  ...
  try {
    const upstream = await fetch(`${TENANT_SVC}/finance/payments/${params.id}`, {
      method: 'GET',
      headers: upstreamHeaders(req, `GET /finance/payments/${params.id}`),
      cache: 'no-store',
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error(`[BFF GET /finance/payments/${params.id}] upstream ${upstream.status}:`, JSON.stringify(body));
      return err('[ERR-RCP-5001]', extractMsg(body, 'Failed to fetch payment receipt'), upstream.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error(`[BFF GET /finance/payments/${params.id}] fetch failed:`, msg);
    return err('[ERR-RCP-5001]', msg, 503);
  }
}
