'use client';

import { useEffect, useMemo, useState } from 'react';

import { AuthGuard } from '../../../components/AuthGuard';
import { bffFetch } from '../../../lib/api';
import type { UserClaims } from '../../../lib/auth';

// ── Reference types ─────────────────────────────────────────────────────────
interface TeacherRef { id: string; firstName: string | null; lastName: string | null; employeeCode: string; }
interface ClassRef   { id: string; name: string; code: string; academicYearId: string; }
interface SectionRef { id: string; name: string; classId: string; className: string; }
interface YearRef    { id: string; name: string; }

// ── Assignment row ───────────────────────────────────────────────────────────
interface Assignment {
  id: string;
  teacherId: string;
  classId: string;
  sectionId: string;
}

interface CreateForm {
  teacherId: string;
  classId: string;
  sectionId: string;
}

interface UpdateForm {
  teacherId: string;
}

type CreateErrors = Partial<Record<keyof CreateForm, string>>;

const EMPTY_FORM: CreateForm = { teacherId: '', classId: '', sectionId: '' };

function validateCreate(f: CreateForm): CreateErrors {
  const e: CreateErrors = {};
  if (!f.teacherId)  e.teacherId = 'Teacher is required.';
  if (!f.classId)    e.classId   = 'Class is required.';
  if (!f.sectionId)  e.sectionId = 'Section is required.';
  return e;
}

// ── Slide-over panel ──────────────────────────────────────────────────────────
function AssignmentPanel({
  open, editing, teachers, years, classes, sections,
  onClose, onSave, saving,
}: {
  open: boolean;
  editing: Assignment | null;
  teachers: TeacherRef[];
  years: YearRef[];
  classes: ClassRef[];
  sections: SectionRef[];
  onClose: () => void;
  onSave: (data: CreateForm | UpdateForm) => void;
  saving: boolean;
}) {
  const isEdit = editing !== null;

  const [form,       setForm]       = useState<CreateForm>(EMPTY_FORM);
  const [updateForm, setUpdateForm] = useState<UpdateForm>({ teacherId: '' });
  const [errors,     setErrors]     = useState<CreateErrors>({});

  useEffect(() => {
    if (open) {
      if (editing) {
        setUpdateForm({ teacherId: editing.teacherId });
      } else {
        setForm(EMPTY_FORM);
      }
      setErrors({});
    }
  }, [open, editing]);

  // Suppress unused variable warning for years (passed for potential future filter UX)
  void years;

  const filteredSections = useMemo(
    () => sections.filter((s) => !form.classId || s.classId === form.classId),
    [sections, form.classId],
  );

  function setField<K extends keyof CreateForm>(k: K, v: CreateForm[K]) {
    setForm((p) => ({
      ...p,
      [k]: v,
      ...(k === 'classId' ? { sectionId: '' } : {}),
    }));
    setErrors((p) => ({ ...p, [k]: undefined }));
  }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (isEdit) {
      if (!updateForm.teacherId) { setErrors({ teacherId: 'Teacher is required.' }); return; }
      onSave(updateForm);
      return;
    }
    const errs = validateCreate(form);
    if (Object.keys(errs).length > 0) { setErrors(errs); return; }
    onSave(form);
  }

  const inputCls = (err?: string) =>
    `w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors
     ${err ? 'border-red-400 bg-red-50' : 'border-slate-300 bg-white hover:border-slate-400'}`;

  function teacherLabel(t: TeacherRef) {
    const name = [t.firstName, t.lastName].filter(Boolean).join(' ') || t.employeeCode;
    return `${name} (${t.employeeCode})`;
  }

  return (
    <>
      <div
        className={`fixed inset-0 bg-black/40 backdrop-blur-sm z-40 transition-opacity duration-300 ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose} aria-hidden="true"
      />
      <div
        role="dialog" aria-label={isEdit ? 'Edit Assignment' : 'New Assignment'} aria-modal="true"
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-50 flex flex-col
          transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              {isEdit ? 'Edit Assignment' : 'Assign Class Teacher'}
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              {isEdit ? 'Reassign to a different teacher.' : 'Assign a teacher to a class section.'}
            </p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close"
            className="rounded-lg p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        </div>

        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-5">
          {isEdit ? (
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                New Teacher <span className="text-red-500">*</span>
              </label>
              <select className={inputCls(errors.teacherId)} value={updateForm.teacherId}
                onChange={(e) => { setUpdateForm({ teacherId: e.target.value }); setErrors({}); }} autoFocus>
                <option value="">— select teacher —</option>
                {teachers.map((t) => (
                  <option key={t.id} value={t.id}>{teacherLabel(t)}</option>
                ))}
              </select>
              {errors.teacherId && <p className="mt-1 text-xs text-red-600">{errors.teacherId}</p>}
            </div>
          ) : (
            <>
              {/* Teacher */}
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                  Teacher <span className="text-red-500">*</span>
                </label>
                <select className={inputCls(errors.teacherId)} value={form.teacherId}
                  onChange={(e) => setField('teacherId', e.target.value)}>
                  <option value="">— select teacher —</option>
                  {teachers.map((t) => (
                    <option key={t.id} value={t.id}>{teacherLabel(t)}</option>
                  ))}
                </select>
                {errors.teacherId && <p className="mt-1 text-xs text-red-600">{errors.teacherId}</p>}
              </div>

              {/* Class */}
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                  Class <span className="text-red-500">*</span>
                </label>
                <select className={inputCls(errors.classId)} value={form.classId}
                  onChange={(e) => setField('classId', e.target.value)}>
                  <option value="">— select class —</option>
                  {classes.map((c) => (
                    <option key={c.id} value={c.id}>{c.name} ({c.code})</option>
                  ))}
                </select>
                {errors.classId && <p className="mt-1 text-xs text-red-600">{errors.classId}</p>}
              </div>

              {/* Section */}
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                  Section <span className="text-red-500">*</span>
                </label>
                <select className={inputCls(errors.sectionId)} value={form.sectionId}
                  onChange={(e) => setField('sectionId', e.target.value)}
                  disabled={!form.classId}>
                  <option value="">— select section —</option>
                  {filteredSections.map((s) => (
                    <option key={s.id} value={s.id}>{s.name}</option>
                  ))}
                </select>
                {errors.sectionId && <p className="mt-1 text-xs text-red-600">{errors.sectionId}</p>}
                {!form.classId && <p className="mt-1 text-xs text-slate-400">Select a class first.</p>}
              </div>
            </>
          )}

          <div className="flex-1" />
          <div className="flex gap-3 pt-4 border-t border-slate-100">
            <button type="button" onClick={onClose} disabled={saving}
              className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60 transition-colors">
              Cancel
            </button>
            <button type="submit" disabled={saving}
              className="flex-1 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60 transition-colors flex items-center justify-center gap-2">
              {saving ? (
                <><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
                </svg>Saving&hellip;</>
              ) : (isEdit ? 'Update' : 'Assign')}
            </button>
          </div>
        </form>
      </div>
    </>
  );
}

// ── Delete dialog ────────────────────────────────────────────────────────────
function DeleteDialog({ open, label, onCancel, onConfirm, deleting }: {
  open: boolean; label: string; onCancel: () => void; onConfirm: () => void; deleting: boolean;
}) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onCancel} aria-hidden="true" />
      <div className="relative z-10 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        <h3 className="text-base font-bold text-slate-900 mb-2">Remove Assignment</h3>
        <p className="text-sm text-slate-600 mb-6">
          Remove class-teacher assignment <span className="font-semibold">&ldquo;{label}&rdquo;</span>?
        </p>
        <div className="flex gap-3">
          <button type="button" onClick={onCancel} disabled={deleting}
            className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60 transition-colors">
            Cancel
          </button>
          <button type="button" onClick={onConfirm} disabled={deleting}
            className="flex-1 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-60 transition-colors">
            {deleting ? 'Removing…' : 'Remove'}
          </button>
        </div>
      </div>
    </div>
  );
}

// ── Main content ─────────────────────────────────────────────────────────────
function ClassTeachersContent({ claims: _claims }: { claims: UserClaims }) {
  const [assignments, setAssignments] = useState<Assignment[]>([]);
  const [teachers,    setTeachers]    = useState<TeacherRef[]>([]);
  const [classes,     setClasses]     = useState<ClassRef[]>([]);
  const [sections,    setSections]    = useState<SectionRef[]>([]);
  const [years,       setYears]       = useState<YearRef[]>([]);

  const [loading,     setLoading]     = useState(true);
  const [loadError,   setLoadError]   = useState('');
  const [panelOpen,   setPanelOpen]   = useState(false);
  const [editing,     setEditing]     = useState<Assignment | null>(null);
  const [saving,      setSaving]      = useState(false);
  const [saveError,   setSaveError]   = useState('');
  const [successMsg,  setSuccessMsg]  = useState('');
  const [deleteTarget, setDeleteTarget] = useState<Assignment | null>(null);
  const [deleting,    setDeleting]    = useState(false);

  useEffect(() => {
    Promise.all([
      bffFetch<Assignment[]>('/api/academic/class-teachers'),
      bffFetch<TeacherRef[]>('/api/faculty'),
      bffFetch<ClassRef[]>('/api/academic-setup/classes'),
      bffFetch<SectionRef[]>('/api/academic-setup/sections'),
      bffFetch<YearRef[]>('/api/academic-setup/years'),
    ])
      .then(([asgns, tchs, cls, secs, yrs]) => {
        setAssignments(Array.isArray(asgns) ? asgns : []);
        setTeachers(   Array.isArray(tchs)  ? tchs  : []);
        setClasses(    Array.isArray(cls)   ? cls   : []);
        setSections(   Array.isArray(secs)  ? secs  : []);
        setYears(      Array.isArray(yrs)   ? yrs   : []);
      })
      .catch((e: unknown) => setLoadError(e instanceof Error ? e.message : 'Failed to load data'))
      .finally(() => setLoading(false));
  }, []);

  const teacherMap  = useMemo(() => new Map(teachers.map((t) => [t.id, t])), [teachers]);
  const classMap    = useMemo(() => new Map(classes.map((c) => [c.id, c.name])), [classes]);
  const sectionMap  = useMemo(() => new Map(sections.map((s) => [s.id, s.name])), [sections]);

  function teacherLabel(id: string) {
    const t = teacherMap.get(id);
    if (!t) return id.slice(0, 8) + '…';
    const name = [t.firstName, t.lastName].filter(Boolean).join(' ') || t.employeeCode;
    return `${name} (${t.employeeCode})`;
  }

  function openAdd()               { setEditing(null); setPanelOpen(true); }
  function openEdit(a: Assignment) { setEditing(a);    setPanelOpen(true); }
  function handleClose()           { setPanelOpen(false); setSaveError(''); setTimeout(() => setEditing(null), 300); }
  function flash(msg: string)      { setSuccessMsg(msg); setTimeout(() => setSuccessMsg(''), 4000); }

  async function handleSave(data: CreateForm | UpdateForm) {
    setSaving(true); setSaveError('');
    try {
      if (editing) {
        await bffFetch<unknown>(`/api/academic/class-teachers/${editing.id}`, {
          method: 'PATCH',
          body: JSON.stringify(data),
        });
        const u = data as UpdateForm;
        setAssignments((prev) =>
          prev.map((a) => a.id === editing.id ? { ...a, teacherId: u.teacherId } : a),
        );
        flash('Assignment updated.');
      } else {
        const d = data as CreateForm;
        const result = await bffFetch<{ id: string }>('/api/academic/class-teachers', {
          method: 'POST',
          body: JSON.stringify(d),
        });
        setAssignments((prev) => [...prev, { ...d, id: result.id }]);
        flash('Class teacher assigned.');
      }
      setSaving(false); setPanelOpen(false); setTimeout(() => setEditing(null), 300);
    } catch (e: unknown) {
      setSaveError(e instanceof Error ? e.message : 'Failed to save'); setSaving(false);
    }
  }

  async function handleDeleteConfirm() {
    if (!deleteTarget) return;
    setDeleting(true);
    try {
      await bffFetch<unknown>(`/api/academic/class-teachers/${deleteTarget.id}`, { method: 'DELETE' });
      setAssignments((prev) => prev.filter((a) => a.id !== deleteTarget.id));
      flash('Assignment removed.');
      setDeleteTarget(null);
    } catch (e: unknown) {
      setLoadError(e instanceof Error ? e.message : 'Failed to delete');
    } finally { setDeleting(false); }
  }

  return (
    <div className="max-w-5xl">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Class Teacher Assignments</h1>
          <p className="text-sm text-slate-500 mt-0.5">Assign a responsible teacher to each class section.</p>
        </div>
        <button type="button" onClick={openAdd}
          className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 transition-colors flex-shrink-0">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
          </svg>
          Assign Teacher
        </button>
      </div>

      {loadError  && <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{loadError}</div>}
      {saveError  && <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{saveError}</div>}
      {successMsg && (
        <div className="mb-4 flex items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3">
          <svg className="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
          <p className="text-sm text-emerald-700">{successMsg}</p>
        </div>
      )}

      {loading && <div className="py-12 text-center text-sm text-slate-400">Loading assignments…</div>}

      {!loading && (
        <>
          <div className="hidden sm:block rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <table className="grand-table w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/70">
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Teacher</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Class</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Section</th>
                  <th className="px-5 py-3.5 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {assignments.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-4 font-medium text-slate-900">{teacherLabel(a.teacherId)}</td>
                    <td className="px-5 py-4 text-slate-600">{classMap.get(a.classId) ?? '—'}</td>
                    <td className="px-5 py-4 text-slate-600">{sectionMap.get(a.sectionId) ?? '—'}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button type="button" onClick={() => openEdit(a)}
                          className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors">
                          Edit
                        </button>
                        <button type="button" onClick={() => setDeleteTarget(a)}
                          className="rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-100 transition-colors">
                          Remove
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
                {assignments.length === 0 && (
                  <tr><td colSpan={4} className="px-5 py-16 text-center text-sm text-slate-400">
                    No assignments yet. Click &ldquo;Assign Teacher&rdquo; to get started.
                  </td></tr>
                )}
              </tbody>
            </table>
          </div>

          <div className="sm:hidden space-y-3">
            {assignments.map((a) => (
              <div key={a.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <p className="font-bold text-slate-900 mb-1">{teacherLabel(a.teacherId)}</p>
                <p className="text-xs text-slate-500 mb-3">
                  {classMap.get(a.classId) ?? '—'} &bull; {sectionMap.get(a.sectionId) ?? '—'}
                </p>
                <div className="flex gap-2">
                  <button type="button" onClick={() => openEdit(a)}
                    className="flex-1 rounded-lg border border-slate-200 bg-white py-2 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors">Edit</button>
                  <button type="button" onClick={() => setDeleteTarget(a)}
                    className="flex-1 rounded-lg border border-red-200 bg-red-50 py-2 text-xs font-medium text-red-600 hover:bg-red-100 transition-colors">Remove</button>
                </div>
              </div>
            ))}
            {assignments.length === 0 && <div className="py-12 text-center text-sm text-slate-400">No assignments yet.</div>}
          </div>

          <p className="mt-4 text-xs text-slate-400">{assignments.length} assignment{assignments.length !== 1 ? 's' : ''}.</p>
        </>
      )}

      <AssignmentPanel
        open={panelOpen} editing={editing}
        teachers={teachers} years={years} classes={classes} sections={sections}
        onClose={handleClose} onSave={handleSave} saving={saving}
      />
      <DeleteDialog
        open={deleteTarget !== null}
        label={deleteTarget ? `${classMap.get(deleteTarget.classId) ?? '?'} / ${sectionMap.get(deleteTarget.sectionId) ?? '?'}` : ''}
        onCancel={() => setDeleteTarget(null)}
        onConfirm={handleDeleteConfirm}
        deleting={deleting}
      />
    </div>
  );
}

export default function ClassTeachersPage() {
  return (
    <AuthGuard>
      {(claims) => <ClassTeachersContent claims={claims} />}
    </AuthGuard>
  );
}
