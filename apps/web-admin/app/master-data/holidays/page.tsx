...
// ── Payload strippers (remove DB-injected `id` etc.) ──────────────────────────
...
/** UTC-safe date key: always yields YYYY-MM-DD regardless of browser timezone */
...
export default function HolidayManagementPage() {
  ...
  // ── Gap 1: Weekend → Matrix auto-sync ────────────────────────────────────
  // Compute which matrix cells are locked by weekend config
  ...
  // Sync matrix when weekend locks change
  useEffect(() => {
  ...
  const loadConfigs = useCallback(async (yearId: string) => {
    ...
    try {
      // Load weekend config
      const wk = await bffFetch<WeekendDay[]>(`/api/web-admin/holidays/weekend-config?academicYearId=${yearId}`);
      ...
      // Load matrix rules
      const mr = await bffFetch<MatrixRule[]>(`/api/web-admin/holidays/matrix-rules?academicYearId=${yearId}`);
      ...
      const hols = await bffFetch<HolidayRow[]>(`/api/web-admin/holidays?academicYearId=${yearId}`);
      setHolidays(Array.isArray(hols) ? hols : []);

      // Load academic calendar entries for reference awareness (Gap 3)
      try {
        const cal = await bffFetch<CalendarEntry[]>(`/api/web-admin/academic-calendar?academicYearId=${yearId}`);
  ...
  const saveWeekendConfig = async () => {
    ...
    try {
      await bffFetch('/api/web-admin/holidays/weekend-config', {
        method: 'POST',
  ...
  const toggleMatrix = (dayOfWeek: string, occurrence: number, field: 'firstHalfOff' | 'secondHalfOff') => {
    // Respect weekend locks — don't toggle if locked by weekend config
  ...
  // Clear Matrix: reset to weekend-config defaults only
  const clearMatrix = () => {
    setMatrixRules((prev) =>
      prev.map((r) => {
  ...
  const saveMatrixRules = async () => {
    // Only send rules that have at least one flag set
    const active = matrixRules.filter((r) => r.firstHalfOff || r.secondHalfOff);
    setSavingMatrix(true);
    try {
      await bffFetch('/api/web-admin/holidays/matrix-rules', {
        method: 'POST',
  ...
  const handlePreview = async () => {
    setPreviewing(true);
    try {
      // Send current (possibly unsaved) weekend + matrix configs for simulation
      const activeMatrix = matrixRules.filter((r) => r.firstHalfOff || r.secondHalfOff);
      const res = await bffFetch<{ holidays: HolidayRow[]; yearName?: string }>('/api/web-admin/holidays/preview', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    try {
      const res = await bffFetch<{ holidays: HolidayRow[]; saved: number }>('/api/web-admin/holidays/generate', {
        method: 'POST',
      ...
      setShowPreview(false);
      setPreviewHolidays([]);
      // Refresh the committed list
      const hols = await bffFetch<HolidayRow[]>(`/api/web-admin/holidays?academicYearId=${selectedYearId}`);
      setHolidays(Array.isArray(hols) ? hols : []);
    } catch (e) {
  ...
  const handleAddHoliday = async () => {
    ...
    try {
      await bffFetch('/api/web-admin/holidays', {
        method: 'POST',
  ...
  const handleUpdateHoliday = async () => {
    ...
    try {
      await bffFetch(`/api/web-admin/holidays/${editId}`, {
        method: 'PATCH',
  ...
  const handleDeleteHoliday = async (id: string) => {
    ...
    try {
      await bffFetch(`/api/web-admin/holidays/${id}`, { method: 'DELETE' });
  ...
  // ── Search filter ──────────────────────────────────────────────────────────
  // Gap 3: Calendar reference rows — holidays from Academic Calendar not yet in DB
  const calendarRefRows = useMemo(() => {
    ...
    for (const entry of sorted) {
      ...
      if (entry.type === 'Vacation_Start') {
        ...
        // Use UTC-safe date iteration to avoid timezone drift
  ...
  // Merge DB holidays + unsaved calendar references
                        ...
                        {weekendDays.map((d) => (
                          <tr key={d.dayOfWeek} className="border-t border-slate-100 hover:bg-slate-50/50">
                            ...
                            <td className="px-3 py-2 text-center">
                              <input
                                type="checkbox"
                                checked={d.isFullHoliday}
                                onChange={() => toggleWeekend(d.dayOfWeek, 'isFullHoliday')}
                            ...
                            <td className="px-3 py-2 text-center">
                              <input
                                type="checkbox"
                                checked={d.firstHalfOff}
                                onChange={() => toggleWeekend(d.dayOfWeek, 'firstHalfOff')}
                                disabled={d.isFullHoliday}
                            ...
                            <td className="px-3 py-2 text-center">
                              <input
                                type="checkbox"
                                checked={d.secondHalfOff}
                                onChange={() => toggleWeekend(d.dayOfWeek, 'secondHalfOff')}
                                disabled={d.isFullHoliday}
                                className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500 disabled:opacity-30"
                              />
                            </td>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={clearMatrix}
                        className="px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 border border-slate-300 rounded-lg hover:bg-slate-200"
                      >
                        Clear Matrix
                      </button>
                      <button
                        onClick={saveMatrixRules}
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-slate-50">
                          ...
                          {OCCURRENCES.map((o) => (
                            <th key={o} colSpan={2} className="text-center px-1 py-2 font-medium text-slate-600">
                              {ordinal(o)}
                            </th>
                          ))}
                        </tr>
                              ...
                              return (
                                <React.Fragment key={o}>
                                  <td className="px-1 py-2 text-center">
                                    <input
                                      type="checkbox"
                                      checked={rule?.firstHalfOff ?? false}
                                      onChange={() => toggleMatrix(d, o, 'firstHalfOff')}
                                      disabled={firstLocked}
                                      className={`w-3.5 h-3.5 rounded focus:ring-purple-500 ${
                                        firstLocked ? 'text-indigo-600 opacity-70 cursor-not-allowed' : 'text-purple-600'
                                      }`}
                                    />
                                  </td>
                                  <td className="px-1 py-2 text-center">
                                    <input
                                      type="checkbox"
                                      checked={rule?.secondHalfOff ?? false}
                                      onChange={() => toggleMatrix(d, o, 'secondHalfOff')}
                                      disabled={secondLocked}
                                      className={`w-3.5 h-3.5 rounded focus:ring-purple-500 ${
                                        secondLocked ? 'text-indigo-600 opacity-70 cursor-not-allowed' : 'text-purple-600'
                                      }`}
                                    />
                                  </td>
                                </React.Fragment>
                  ...
                  <div className="bg-amber-50 border border-amber-300 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-amber-700 mt-0.5">
                        Please upload the Academic Calendar for this Academic Year before generating holidays.
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    <div className="flex gap-3 ml-auto">
                      <button
                        ...
                        disabled={previewing || calendarEntries.length === 0}
                        className="px-4 py-2 text-sm font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg hover:bg-indigo-100 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {previewing ? 'Loading Preview...' : 'Preview Holidays'}
                      </button>
                      <button
                        onClick={handleGenerate}
                        disabled={generating || calendarEntries.length === 0}
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-amber-200 p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-lg font-semibold text-amber-700">
                        Preview ({previewHolidays.length} entries)
                      </h2>
                      <button
                          ...
                          {previewHolidays.map((h, i) => (
                            <tr key={i} className="border-t border-slate-100 hover:bg-slate-50/50">
                              ...
                              <td className="px-3 py-2">
                                <span className={`px-2 py-0.5 text-xs font-medium rounded-full border ${typeBadge(h.type)}`}>
                                  {h.type}
                                </span>
                              </td>
                              <td className="px-3 py-2 text-slate-600">{halfLabel(h)}</td>
                              <td className="px-3 py-2">
                                <span className={`px-2 py-0.5 text-xs font-medium rounded-full border ${sourceBadge(h.source)}`}>
                                  {h.source}
                                </span>
                              </td>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  {!loadingHolidays && mergedHolidays.length === 0 && (
                    <p className="text-sm text-slate-500 py-8 text-center">
                      No holiday entries yet. Configure weekends &amp; matrix rules above, then click &quot;Generate &amp; Save&quot;.
                    </p>
                  )}
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {filtered.map((h, i) => {
                            ...
                            return (
                              <tr key={h.id ?? `ref-${i}`} className={`border-t border-slate-100 hover:bg-slate-50/50 ${
                                isCalendarRef ? 'bg-amber-50/50' : ''
                              }`}>
                                <td className="px-3 py-2 text-slate-500">{i + 1}</td>
                                <td className="px-3 py-2 text-slate-700 whitespace-nowrap">{fmtDate(h.date)}</td>
                                <td className="px-3 py-2 text-slate-500 whitespace-nowrap">{dayName(h.date)}</td>
                                ...
                                <td className="px-3 py-2">
                                  ...
                                  {isCalendarRef && (
                                    <span className="ml-1 px-1.5 py-0.5 text-xs font-medium bg-amber-100 text-amber-700 rounded border border-amber-300">
                                      Unsaved
                                    </span>
                                  )}
                                </td>
              ...
              <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setAddForm((f) => ({ ...f, occasion: e.target.value }))}
                      placeholder="e.g. Rainy Day Holiday"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-300"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-indigo-300"
                    >
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 text-sm text-slate-600">
                      <input
                        type="radio"
                        name="addHalf"
                        checked={addForm.isFullDay}
                        onChange={() => setAddForm((f) => ({ ...f, isFullDay: true, isFirstHalf: false, isSecondHalf: false }))}
                    ...
                    <label className="flex items-center gap-2 text-sm text-slate-600">
                      <input
                        type="radio"
                        name="addHalf"
                        checked={addForm.isFirstHalf}
                        onChange={() => setAddForm((f) => ({ ...f, isFullDay: false, isFirstHalf: true, isSecondHalf: false }))}
                        className="text-indigo-600"
                      />
                      First Half
                    </label>
                    <label className="flex items-center gap-2 text-sm text-slate-600">
                      <input
                        type="radio"
                        name="addHalf"
                        checked={addForm.isSecondHalf}
                        onChange={() => setAddForm((f) => ({ ...f, isFullDay: false, isFirstHalf: false, isSecondHalf: true }))}
                        className="text-indigo-600"
                      />
                      Second Half
                    </label>
                ...
                <div className="flex justify-end gap-3 mt-6">
                  ...
                  <button
                    ...
                    disabled={addingHoliday || !addForm.date || !addForm.occasion}
                    className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50"
                  >
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 text-sm text-slate-600">
                      <input
                        ...
                        onChange={() => setEditForm((f) => ({ ...f, isFullDay: true, isFirstHalf: false, isSecondHalf: false }))}
                        className="text-indigo-600"
                      />
                      Full Day
                    </label>
                    <label className="flex items-center gap-2 text-sm text-slate-600">
                      <input
                        type="radio"
                        name="editHalf"
                        checked={editForm.isFirstHalf}
                        onChange={() => setEditForm((f) => ({ ...f, isFullDay: false, isFirstHalf: true, isSecondHalf: false }))}
                        className="text-indigo-600"
                      />
                      First Half
                    </label>
                    <label className="flex items-center gap-2 text-sm text-slate-600">
                      <input
                        type="radio"
                        name="editHalf"
                        checked={editForm.isSecondHalf}
                        onChange={() => setEditForm((f) => ({ ...f, isFullDay: false, isFirstHalf: false, isSecondHalf: true }))}
                        className="text-indigo-600"
                      />
                      Second Half
                    </label>
