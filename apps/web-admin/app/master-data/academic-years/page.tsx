        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              {isEdit ? 'Update the master template academic year.' : 'Define a new master template academic session.'}
            </p>
          </div>
          <button
            type="button"
          ...
          {form.isActive && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex items-start gap-2">
              ...
              <p className="text-xs text-amber-700">
                ...
                for all schools using this template.
              </p>
            </div>
          )}
...
// ── Main content ──────────────────────────────────────────────────────────────
function MasterAcademicYearsContent() {
  ...
  async function handleSave(form: YearForm) {
    ...
    try {
      if (editingYear) {
        await bffFetch(`/api/web-admin/academic-years/${editingYear.id}`, {
          method: 'PATCH',
          body: JSON.stringify({
            name: form.name.trim(),
        ...
        await bffFetch('/api/web-admin/academic-years', {
          method: 'POST',
          body: JSON.stringify({
            name: form.name.trim(),
            startDate: form.startDate,
  ...
  async function handleSeed() {
    ...
    setErrorMsg('');
    try {
      await bffFetch<{ seeded: number }>('/api/web-admin/academic-years/seed', { method: 'POST' });
  ...
  async function handleDelete(year: AcademicYear) {
    ...
    setErrorMsg('');
    try {
      await bffFetch(`/api/web-admin/academic-years/${year.id}`, { method: 'DELETE' });
    ...
    <div className="max-w-5xl">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage master template academic sessions and term dates.</p>
        </div>
...
// ── Page export ───────────────────────────────────────────────────────────────
export default function MasterAcademicYearsPage() {
  return (
    <AuthGuard>
      {(claims) => {
        ...
        return <MasterAcademicYearsContent />;
      }}
    </AuthGuard>
  );
}
