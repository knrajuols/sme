        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              {isEdit ? 'Update this master template subject.' : 'Define a new master template subject.'}
            </p>
          </div>
          <button
            type="button"
...
// ── Main content ──────────────────────────────────────────────────────────────
function MasterSubjectsContent() {
  ...
  useEffect(() => {
    setLoading(true);
    bffFetch<Subject[]>('/api/web-admin/subjects')
      .then((subj) => {
  ...
  async function handleSeed() {
    ...
    setSaveError('');
    try {
      const result = await bffFetch<{ seeded: number }>('/api/web-admin/subjects/seed', {
        method: 'POST',
      });
      const fresh = await bffFetch<Subject[]>('/api/web-admin/subjects');
      setSubjects(Array.isArray(fresh) ? fresh : []);
      setSuccessMsg(`${result.seeded} master template subjects generated successfully.`);
  ...
  async function handleStatusToggle(s: Subject) {
    ...
    setTogglingId(s.id);
    try {
      await bffFetch<unknown>(`/api/web-admin/subjects/${s.id}/status`, {
        method: 'PATCH',
  ...
  async function handleSave(form: SubjectForm) {
    ...
    try {
      if (editingSubject) {
        await bffFetch<unknown>(`/api/web-admin/subjects/${editingSubject.id}`, {
          method: 'PATCH',
        ...
        const result = await bffFetch<{ id: string }>('/api/web-admin/subjects', {
          method: 'POST',
          body: JSON.stringify({ name: form.name.trim(), code: form.code.trim().toUpperCase() }),
        });
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleting(true);
    try {
      await bffFetch<unknown>(`/api/web-admin/subjects/${deleteTarget.id}`, { method: 'DELETE' });
    ...
    <div className="max-w-5xl">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage master template curriculum subjects.</p>
        </div>
      ...
      {loading && (
        <div className="py-12 text-center text-sm text-slate-400">Loading subjects&hellip;</div>
      )}
