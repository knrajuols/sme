  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <p className="text-sm text-slate-500 mb-5">Select how the 10 master template sections will be named.</p>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              {isEdit ? 'Update this master template section.' : 'Create a new master template section.'}
            </p>
          </div>
          <button
            type="button"
...
// ── Main content ──────────────────────────────────────────────────────────────
function MasterSectionsContent() {
  ...
  useEffect(() => {
    setLoading(true);
    bffFetch<Section[]>('/api/web-admin/sections')
      .then((sec) => {
  ...
  async function handleSeed(style: NamingStyle) {
    ...
    setSaveError('');
    try {
      const result = await bffFetch<{ seeded: number }>('/api/web-admin/sections/seed', {
        method: 'POST',
      ...
      const fresh = await bffFetch<Section[]>('/api/web-admin/sections');
  ...
  async function handleSave(form: SectionForm) {
    ...
    try {
      if (editingSection) {
        await bffFetch<unknown>(`/api/web-admin/sections/${editingSection.id}`, {
          method: 'PATCH',
        ...
        const result = await bffFetch<{ id: string }>('/api/web-admin/sections', {
          method: 'POST',
          body: JSON.stringify({ name: form.name.trim() }),
        });
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleting(true);
    try {
      await bffFetch<unknown>(`/api/web-admin/sections/${deleteTarget.id}`, { method: 'DELETE' });
    ...
    <div className="max-w-5xl">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage master template section names that can be assigned to any class.</p>
        </div>
      ...
      {loading && (
        <div className="py-12 text-center text-sm text-slate-400">Loading sections&hellip;</div>
      )}
