...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!upstream.ok) {
      console.error('[BFF POST /web-admin/classes/seed] upstream', upstream.status, JSON.stringify(body));
      return err('[ERR-WA-CLS-5005]', extractMsg(body, 'Failed to seed master classes'), upstream.status);
    }
    return NextResponse.json(body, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /web-admin/classes/seed] fetch failed:', msg);
    return err('[ERR-WA-CLS-5005]', msg, 503);
  }
}
