...
// ── POST /api/web-admin/subjects/seed ─────────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    const upstream = await fetch(`${TENANT_SVC}/web-admin/subjects/seed`, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /web-admin/subjects/seed'),
    });
    const body: unknown = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error('[BFF POST /web-admin/subjects/seed] upstream', upstream.status, JSON.stringify(body));
      return err('[ERR-WA-SUB-5005]', extractMsg(body, 'Failed to seed master subjects'), upstream.status);
    }
    return NextResponse.json(body, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /web-admin/subjects/seed] fetch failed:', msg);
    return err('[ERR-WA-SUB-5005]', msg, 503);
  }
}
