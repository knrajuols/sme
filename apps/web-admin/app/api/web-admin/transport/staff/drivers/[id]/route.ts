...
export async function PATCH(req: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  let payload: unknown;
  try { payload = await req.json(); } catch { return err('[ERR-TR-DR-5003]', 'Invalid JSON', 400); }
  try {
    ...
    if (!up.ok) return err('[ERR-TR-DR-5003]', extractMsg(body, 'Failed to update driver'), up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) { return err('[ERR-TR-DR-5003]', e instanceof Error ? e.message : 'Unavailable', 503); }
}
...
export async function DELETE(req: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    ...
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) return err('[ERR-TR-DR-5004]', extractMsg(body, 'Failed to delete driver'), up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) { return err('[ERR-TR-DR-5004]', e instanceof Error ? e.message : 'Unavailable', 503); }
}
