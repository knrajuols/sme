...
// ── POST /api/web-admin/holidays/preview ────────────────────────────────────
export async function POST(req: NextRequest): Promise<NextResponse> {
  const url = `${TENANT_SVC}/web-admin/holidays/preview`;
  try {
    const payload = await req.json();
    const up = await fetch(url, {
      method: 'POST',
      headers: upstreamHeaders(req, 'POST /web-admin/holidays/preview'),
      body: JSON.stringify(payload),
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) {
      console.error('[BFF POST /holidays/preview] upstream', up.status, JSON.stringify(body));
      return err('[ERR-WA-HOL-PRV-5001]', extractMsg(body, 'Failed to preview holidays'), up.status);
    }
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /holidays/preview] fetch failed:', msg);
    return err('[ERR-WA-HOL-PRV-5001]', msg, 503);
  }
}
