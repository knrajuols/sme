...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!upstream.ok) {
      console.error('[BFF POST /web-admin/fee-structures/seed] upstream', upstream.status, JSON.stringify(body));
      return err('[ERR-WA-FSD-5001]', extractMsg(body, 'Failed to seed fee structures'), upstream.status);
    }
    return NextResponse.json(body, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Tenant service unavailable';
    console.error('[BFF POST /web-admin/fee-structures/seed] fetch failed:', msg);
    return err('[ERR-WA-FSD-5001]', msg, 503);
  }
}
