...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    const up = await fetch(`${TENANT_SVC}/web-admin/transport-allocations/students-grid`, {
      method: 'GET',
      headers: upstreamHeaders(req),
      cache: 'no-store',
    });
    const body: unknown = await up.json().catch(() => ({}));
    if (!up.ok) return err('[ERR-TR-AL-5007]', 'Failed to load students grid', up.status);
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    return err('[ERR-TR-AL-5007]', e instanceof Error ? e.message : 'Unavailable', 503);
  }
}
