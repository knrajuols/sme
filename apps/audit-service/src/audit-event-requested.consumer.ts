...
@Injectable()
export class AuditEventRequestedConsumer
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queueName = 'audit-service.queue';
  ...
  private async processMessage(message: any): Promise<void> {
    ...
    try {
      const envelope = JSON.parse(message.content.toString('utf8')) as EventEnvelope<AuditEventRequestedPayload>;
      await this.appService.handleAuditEventRequested(envelope);
